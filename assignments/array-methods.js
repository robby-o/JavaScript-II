...
runners.map(function (names) {
  return firstNamesAllCaps.push(names.first_name.toUpperCase())
});
...
let runnersSizeShirt = [];
runnersSizeShirt = runners.filter(function (items) {
  return items.shirt_size == "L" || items.shirt_size == "XS"
});
console.log(runnersSizeShirt);

// Problem 2

let donationDoubled = [];
donationDoubled = runners.map(items => items.donation * 2);
console.log(donationDoubled);

// Problem 3
let averageDonation = [];
averageDonation = runners.reduce(function (accumulator, items) {
  return accumulator + items.donation;
}, 0);
